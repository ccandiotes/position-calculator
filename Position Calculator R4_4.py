...
APP_TITLE = "Position Calculator R4"
...
class Mode1Window(tk.Toplevel, PreviewAndMappingMixin):
    ...
    def on_compute(self):
        ...
        installed_depth_md = toe_deepest[1]
        ...
        try:
            with open(out_path, 'w', encoding='utf-8-sig', newline='') as f:
                ...
                for line in summary_text.splitlines():
                    w.writerow(["#", line])
                w.writerow([])
                w.writerow(["Instrument #", "MD (m)", "X", "Y", "Z"])
...
class Mode2Window(tk.Toplevel, PreviewAndMappingMixin):
    ...
    def on_compute(self):
        ...
        summary = (
            ...
            f"Hole depth (MD collar→toe): {toe_md:.3f} m\n"
            f"Installed depth (MD collar→Toe Instrument): {installed_depth_md:.3f} m\n"
            f"Toe Instrument (deepest): X:{first[2]:.3f}, Y:{first[3]:.3f}, Z:{first[4]:.3f}\n"
            f"Top Instrument (closest to collar): X:{last[2]:.3f}, Y:{last[3]:.3f}, Z:{last[4]:.3f}\n"
        ...
        try:
            with open(out_path, 'w', encoding='utf-8-sig', newline='') as f:
                ...
                for nr, md, x, y, z in export_rows:
                    w.writerow([nr, f"{md:.3f}", f"{x:.3f}", f"{y:.3f}", f"{z:.3f}"])
            messagebox.showinfo("Exported", f"Wrote: {out_path}", parent=self)
        except Exception as e:
            messagebox.showerror("Write error", f"Could not write output.\n\n{e}", parent=self)
...
class Mode3Window(tk.Toplevel):
    ...
    def _build_ui(self):
        # NOTE: No "Select Survey CSV" button in Mode3 by design (R4 change).
    ...
    def on_compute(self):
        ...
        hole = (self.hole_name_var.get() or "").strip()
        installed_depth_md = toe_by_md[0]  # <-- FIX: was toe_deepest[1]
...
class Mode4Window(tk.Toplevel):
    ...
    def on_compute(self):
        ...
        hole = (self.hole_name_var.get() or "").strip()
        installed_depth_md = toe_by_md[0]
